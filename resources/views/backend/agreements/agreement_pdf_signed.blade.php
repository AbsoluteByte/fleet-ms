...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .company-block .co-name { font-size: 12px; font-weight: bold; }
        .company-block .co-email { font-size: 11px; }

        .details-wrap { display: table; width: 100%; margin-bottom: 6px; }
        ...
        .col-title { font-weight: bold; font-size: 11px; margin-bottom: 5px; }
        .field { margin-bottom: 3px; font-size: 10.5px; }
        .field .lbl { font-weight: bold; }

        .prev-car { text-align: right; font-weight: bold; font-size: 10.5px; margin-bottom: 5px; }
        ...
        .cond-list { padding-left: 20px; margin: 6px 0; }
        .cond-list li { margin-bottom: 4px; text-align: justify; font-size: 10.5px; }

        .deposit-note { text-align: center; font-size: 10.5px; margin: 8px 0; }
        ...
        .car-grid { width: 75%; margin: 6px auto; border-collapse: collapse; }
        .car-grid td { width: 50%; text-align: center; padding: 4px; vertical-align: middle; }
        .car-grid img { max-width: 100%; max-height: 90px; }
        .car-placeholder { border: 1px dashed #bbb; height: 80px; line-height: 80px; font-size: 9px; color: #aaa; }

        .check-text { font-size: 10.5px; font-weight: bold; margin: 8px 0 5px 0; }
        .agree-text { font-size: 10.5px; text-align: justify; margin-bottom: 14px; }
        ...
        .sig-container { position: relative; height: 45px; margin-bottom: 3px; }
        .sig-img {
            max-width: 90%; max-height: 40px;
            position: absolute; bottom: 0; left: 50%;
            transform: translateX(-50%);
            display: block;
        }
        .sig-underline { border-bottom: 1px solid #000; width: 100%; position: absolute; bottom: 0; }
        .sig-label { font-size: 10.5px; font-weight: bold; }
        .sig-info  { font-size: 8px; color: #555; margin-top: 2px; }
    </style>
</head>
<body>

{{-- ── BIG TITLE ── --}}
<div class="doc-title">Hire Agreement &mdash; SIGNED COPY</div>
...
<div class="deposit-note">DEPOSITS WILL BE REFUNDED AFTER 5 WORKING DAYS WHEN THE HIRE VEHICLE IS RETURNED.</div>
...
<div class="check-text">CHECK BEFORE TAKING VEHICLE IN ANY TYRES OR BULBS ARE REQUIRED, AS NONE WILL BE PROVIDED AT A LATER DATE.</div>

<div class="agree-text">I have read and understand ALL the above conditions of hire and I agree to abide by them. I agree with the above vehicle condition report and I agree the vehicle is ONLY to be driven by the person(s) named above who have signed this form.</div>
...
<table class="sig-table">
    <tr>
        <td>
            <div class="sig-container">
                @if(isset($signature_image) && $signature_image)
                    <img src="{{ $signature_image }}" class="sig-img" alt="Client Signature">
                @endif
                <div class="sig-underline"></div>
            </div>
            <div class="sig-label">Client</div>
            @if(isset($signature_image) && $signature_image)
                <div class="sig-info">{{ $driver->full_name }} | {{ now()->format('d/m/Y H:i') }}</div>
            @endif
        </td>
        <td>
        ...
        <td>
            <div class="sig-container">
                @if(isset($signature_image) && $signature_image)
                    <div style="position:absolute; bottom:4px; width:100%; text-align:center; font-size:10.5px;">{{ now()->format('d/m/Y') }}</div>
                @endif
                <div class="sig-underline"></div>
            </div>
...
<div style="text-align:center; margin-top:12px; font-size:9px; color:#666; border-top:1px solid #ddd; padding-top:6px;">
    <strong>Electronically Signed</strong> | Agreement ID: {{ $agreement->id }} | Signed: {{ now()->format('d/m/Y H:i:s') }}
</div>

{{-- ================================================================
     PAGE 2 — STATEMENT OF UNDERSTANDING (SIGNED)
     ================================================================ --}}
<div style="page-break-before:always; padding:25px 30px; font-family:Arial,sans-serif; font-size:10.5px; line-height:1.45; color:#000;">
    ...
    <div style="border-top:1px solid #000; padding-top:12px;">
        ...
        <table style="width:100%; margin-top:18px; border-collapse:collapse;">
            <tr>
                <td style="width:60%; padding-right:15px; vertical-align:bottom;">
                    <strong>SIGN:</strong>
                    <div style="position:relative; display:inline-block; width:72%; vertical-align:bottom;">
                        @if(isset($signature_image) && $signature_image)
                            <img src="{{ $signature_image }}" style="max-width:100%; max-height:36px; display:block;" alt="Signature">
                        @endif
                        <div style="border-bottom:1px solid #000; width:100%;"></div>
                        @if(isset($signature_image) && $signature_image)
                            <div style="font-size:8px; color:#555; margin-top:2px;">{{ $driver->full_name }} | {{ now()->format('d/m/Y H:i') }}</div>
                        @endif
                    </div>
                </td>
                <td style="width:40%; vertical-align:bottom;">
                    <strong>DATE:</strong>
                    <div style="display:inline-block; width:68%; vertical-align:bottom;">
                        @if(isset($signature_image) && $signature_image)
                            <div style="font-size:10.5px; padding-bottom:1px;">{{ now()->format('d/m/Y') }}</div>
                        @endif
                        <div style="border-bottom:1px solid #000; width:100%;"></div>
                    </div>
                </td>
    ...
    <div style="text-align:center; margin-top:14px; font-size:9px; color:#666; border-top:1px solid #ddd; padding-top:6px;">
        <strong>Electronically Signed</strong> | Agreement ID: {{ $agreement->id }} | Statement of Understanding | {{ now()->format('d/m/Y H:i:s') }}
    </div>
</div>

{{-- ================================================================
     PAGE 3 — STATEMENT OF LIABILITY (SIGNED)
     ================================================================ --}}
<div style="page-break-before:always; padding:25px 30px; font-family:Arial,sans-serif; font-size:10.5px; line-height:1.45; color:#000;">
    ...
    <div style="text-align:center; margin-top:22px;">
        <div style="display:inline-block; width:45%; text-align:center;">
            @if(isset($signature_image) && $signature_image)
                <img src="{{ $signature_image }}" style="max-width:90%; max-height:38px; display:block; margin:0 auto;" alt="Signature">
            @endif
            <div style="border-bottom:1px solid #000; width:100%; margin-bottom:4px;"></div>
            <strong>CLIENT SIGNATURE</strong>
            @if(isset($signature_image) && $signature_image)
                <div style="font-size:8px; color:#555; margin-top:3px;">{{ $driver->full_name }} | {{ now()->format('d/m/Y H:i') }}</div>
            @endif
        </div>
    </div>

    <div style="text-align:center; margin-top:20px; font-size:9px; color:#666; border-top:1px solid #ddd; padding-top:6px;">
        <strong>Electronically Signed</strong> | Agreement ID: {{ $agreement->id }} | Statement of Liability | {{ now()->format('d/m/Y H:i:s') }}
    </div>
</div>
